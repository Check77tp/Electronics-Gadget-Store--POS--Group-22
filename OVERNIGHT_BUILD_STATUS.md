...
1. **Click through every screen in a real browser** as both an admin/manager and a cashier login, and fix anything that looks visually wrong — this is the one verification step this environment couldn't do itself.
...
3. **Set up version control** (git init + initial commit) if this hasn't been done already outside this session — `iteration_plan.md` also lists "Git repository with commit history" as a Construction deliverable, and this build was done in a plain directory with no VCS.

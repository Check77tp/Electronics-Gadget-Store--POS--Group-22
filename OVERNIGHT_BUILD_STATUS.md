...
- **Product images added, all 20 SKUs, real photos.** The `image_url` field already existed on `Product` and every screen already rendered it when present (POS grid, Products table, Inventory table) — it was just never populated, so every item fell back to a generic icon. Samson asked for real product photos (not generated placeholders or generic category stock photos, having been offered all three), so each SKU was matched to a real, freely-licensed photo from Wikimedia Commons and Wikipedia, every URL individually verified with a live HTTP request before use. Added a `PRODUCT_IMAGES` dict to `backend/seed.py` (keyed by SKU, used on fresh seeds) and pushed the same URLs to the currently-running dev database via `PUT /api/products/{id}` (so this demo's existing data — including its 2 completed sales — didn't have to be wiped and reseeded). **Caveat, worth knowing:** Commons doesn't have a photo of every exact model/color/capacity in the seed data, so roughly half use the closest real photo of the same product line instead of an exact match (e.g. the Sony headphones show a WH-1000XM3 since XM5 isn't on Commons yet; the Keychron K2 shows a K8; the Razer mouse shows a DeathAdder Elite, not V3). Each substitution is commented inline in `seed.py`'s `PRODUCT_IMAGES` dict. If exact-model accuracy matters more than having *a* real photo, the fix is a one-line `image_url` edit per product (via the Add/Edit Product form's Image URL field, or directly in `seed.py`).

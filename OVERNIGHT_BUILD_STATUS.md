...
- **Dashboard's "Quick Actions" panel isn't role-gated** (`frontend/src/components/dashboard/QuickActions.jsx`). Every other manager-only surface in the app (widgets, whole pages) checks `hasRole('admin', 'manager')` before rendering, but this one component always shows all 4 tiles including "View Reports" and "Manage Inventory" to a cashier. Clicking "View Reports" as a cashier leads straight into the "Not Authorized" dead-end screen, and "Manage Inventory" overstates what a cashier can actually do there (view-only, no adjust/export). Small, easy fix — filter `ACTIONS` by role before rendering — but worth doing since it's the only place in the app where the UI doesn't match what the backend will actually allow.
- **Products table overflows horizontally on a standard 1440px laptop screen** (`frontend/src/components/products/ProductsTable.jsx`). Measured the scrollable table container at 1247px content width vs. 1136px visible width, so the rightmost "Actions" column (Edit / Discontinue buttons) is scrolled out of view with no visible scrollbar or shadow hinting that there's more to the right. A user on a laptop-sized screen has to know to scroll the table sideways to edit or discontinue a product. The Inventory table handles a similar number of columns without this problem, so it's likely just a couple of columns (Barcode + SKU are both shown at full width) that could be trimmed or made narrower.
